...
- `check` command for readiness diagnostics (Node, Git, config, baseline, optional LLM inspect)
- Interactive `setup` wizard for repository config, first scan, and operator LLM profile defaults
- Operator profile support (`~/.config/r3-doctor/profile.json`) for LLM defaults per ADR 0005
...
- Setup wizard consolidates config prompts and defers baseline save when the worktree cannot accept commit-bound baselines

### Fixed

- Human-readable reports no longer hide score meaning, calibration status, or top actionable next steps
- Baseline save failures now explain remediation instead of misleading `config error` messages
- Setup wizard no longer asks redundant overwrite/write config questions
...
[0.2.0]: https://github.com/kaz-toc/r3-doctor/releases/tag/v0.2.0
